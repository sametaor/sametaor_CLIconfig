...
#include 
std::string val;
std::vector my_vector; // initialize the vector
std::cin >> val;
...
vector::iterator it; // initialize the iterator for vector
...
#include
std::set ST;    // Will initialize the set of int data type
...
#include
std::map mymap;  // Will initialize the map with key as char and value as int
...
std::sort(tester.begin(), tester.end(), [](const pair& lhs, const pair& rhs) {
