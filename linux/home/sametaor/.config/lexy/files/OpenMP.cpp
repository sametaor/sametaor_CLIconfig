...
int main() {
    ...
    std::vector a(num_elements, 1.0);
    std::vector b(num_elements, 2.0);
    std::vector c(num_elements, 0.0);
    ...
    auto duration_serial = std::chrono::duration_cast(end_time - start_time).count();
    ...
    auto duration_parallel = std::chrono::duration_cast(end_time - start_time).count();

    std::cout << "Serial execution time: " << duration_serial << " ms" << std::endl;
    std::cout << "Parallel execution time: " << duration_parallel << " ms" << std::endl;
    std::cout << "Speedup: " << static_cast(duration_serial) / duration_parallel << std::endl;
...
int mandelbrot(const std::complex &c) {
    std::complex z = c;
    int n = 0;
...
int main() {
    std::vector> values(height, std::vector(width));
