...
#
jq -n '123 | .[0]'     # => jq: error (at ): Cannot index number with number
jq -n '"abc" | .name'  # => jq: error (at ): Cannot index string with string "name"
jq -n '{"a": 97} | .[0]'    # => jq: error (at ): Cannot index object with number
jq -n '[89, 64] | .["key"]' # => jq: error (at ): Cannot index array with string "key"
