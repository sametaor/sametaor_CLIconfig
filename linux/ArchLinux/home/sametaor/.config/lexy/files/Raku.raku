...
say %hash; # OUTPUT: «True␤», gets value associated to key 'is-even'
...
# For example, the `is` trait is actually a `multi sub` named `trait_mod:`,
# and it works off that. Thus, `is rw`, is simply a dispatch to a function with
# this signature `sub trait_mod:(Routine $r, :$rw!) {}`
...
prefix:("The Queen"); # OUTPUT: «The Queen Won!␤»
...
sub infix:( Int $n, Block $r ) { # infix ('between')
    for ^$n {
...
# and `.Str` methods but you can also just use `match` for hash access
