...
fn id5 {a:type} (x: a) = id2 x // <> for template parameters
...
fun terminating_factorial {n:nat} .. (n: int n): int =
...
// (proof | value)
fun fib {n:nat} .. (n: int n): [f:int] (Fib(n,f) | int f) =
...
prfun append_total {x,y: IntList} .. (): [z:IntList] ILAppend(x,y,z)
  ...
  let
    ...
    | ptr: ptr l,
      ...
      then acc
        ...
        val result = loop(rest | ptr_add(ptr, 1), length - 1, acc + !ptr)
...
fn refined_square {n:int} (x: ∫ n >> int (n*n)): void =
  x := x * x
...
extern fn scanf (format: string, value: ∫): int = "scanf_wrapper"
