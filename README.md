...
| Zsh-completions | Extra tab completions  | `yay -S zsh-completions`                | `sudo apt install zsh-common`* | Manual                      | Manual                     | Manual                   | Manual                        | Manual              | Manual                                   | Manual                 |
...
| Man-db      | Man page utilities         | `sudo pacman -S man-db`                 | `sudo apt install man-db`      | `sudo dnf install man-db`   | `sudo zypper install man`  | `apk add man`            | `nix-env -iA nixos.man-db`    | `emerge man-db`     | Manual                                   | `pkg install man`      |
...
| Lolcat      | Rainbow text generator     | `sudo pacman -S lolcat`                 | `sudo apt install lolcat`**    | Gem: `gem install lolcat`   | Gem: `gem install lolcat`  | Manual                   | `nix-env -iA nixos.lolcat`    | `emerge lolcat`     | Manual                                   | Manual                 |
...
| Btop           | Resource monitor              | `sudo pacman -S btop`         | `sudo apt install btop`     | `sudo dnf install btop`     | `sudo zypper install btop` | `apk add btop`   | `nix-env -iA nixos.btop` | `emerge btop`      | `winget install aristocratos.btop` / `scoop install btop` / `choco install btop` | Manual            |
| Systemd        | Init/system manager           | Native/preinstalled           | `sudo apt install systemd`  | Included                    | Included             | Manual           | Included                | Included          | WSL/WSL2 (auto, see docs)     | N/A               |
| Loginctl       | Session controller (part of systemd) | Native/preinstalled     | In systemd                  | In systemd                  | In systemd           | Manual           | In systemd               | In systemd         | N/A                          | N/A               |
| Reflector      | Pacman mirror optimizer (Arch) | `sudo pacman -S reflector`    | Manual (pip/cargo)          | Manual                      | Manual               | Manual           | Manual                  | Manual            | N/A                           | N/A               |
| Pkgtop         | TUI Package monitor           | `yay -S pkgtop` (AUR)         | `cargo install pkgtop`      | `cargo install pkgtop`      | `cargo install pkgtop` | `cargo install pkgtop` | `cargo install pkgtop` | `cargo install pkgtop` | N/A                       | N/A               |
| Oh-my-posh     | Shell prompt theme engine     | `yay -S oh-my-posh-bin`       | `snap install oh-my-posh` (Snap, newer IoT) | `rpm` or manual binary | Manual/init script  | Manual           | Manual                  | Manual            | `winget install JanDeDobbeleer.OhMyPosh`/`scoop install oh-my-posh`/`choco install oh-my-posh` | `pkg install oh-my-posh` (if available) |
| CommandNotFound| Suggest missing apt packages  | N/A (not needed)              | `sudo apt install command-not-found` | N/A                  | N/A                | N/A              | N/A                     | N/A               | N/A                           | N/A               |
...
| Yazi           | Modern terminal file manager  | `yay -S yazi`                 | `cargo install yazi-fm`     | `cargo install yazi-fm`     | `cargo install yazi-fm` | `cargo install yazi-fm` | `cargo install yazi-fm` | `cargo install yazi-fm` | `scoop install yazi`               | Manual or Termux user bucket  |
...
| Visual Studio Code | Editor (VSCode OSS & MS)   | `yay -S visual-studio-code-bin`/`vscodium-bin` | Download .deb/`apt install code` | Download .rpm/`dnf install code` | Download .rpm/`zypper install code` | Manual | `nix-env -iA nixos.vscode`/`vscodium` | Manual | `winget install Microsoft.VisualStudioCode`/`scoop install vscode`/`choco install vscode` | Termux (with `code-server`)  |
...
| RubyGems     | Ruby package manager        | `sudo pacman -S ruby`      | `sudo apt install ruby` | `sudo dnf install ruby` | `sudo zypper install ruby` | `apk add ruby`         | `nix-env -iA nixos.ruby`       | `emerge ruby`     | `winget install RubyInstallerTeam.Ruby`/`choco install ruby`      | `pkg install ruby`     |
...
| pip install  | PyPI universal (any system) | N/A                        | N/A                    | N/A                     | N/A                   | N/A                | N/A                      | N/A               | Manual                                                 | `pip install <pkg>`    |
...
| Eza/Exa               | Modern ls replacement              | `winget install eza-community.eza`         | `scoop install eza`         | `choco install eza`          | N/A                       |
...
| x64dbg                | Debugger/disassembler              | `winget install x64dbg.x64dbg`              | `scoop install x64dbg`      | `choco install x64dbg`       | N/A                       |
...
| Zen Browser         | Chromium browser alt                     | N/A                               | N/A                            | N/A                          | [Official download/manual]|
...
| Spotify             | Streaming app                            | `winget install Spotify.Spotify`   | `scoop install spotify`         | `choco install spotify`      |                             |
...
| Playnite            | All-in-one game launcher                 | `winget install Playnite.Playnite` | `scoop install playnite`        | `choco install playnite`     |                             |
...
| Nilesoft Shell      | Extended context menu utility            | `winget install Nilesoft.Shell`    | N/A                            | N/A                          |                             |
...
| wget         | Download util                              | `pkg install wget`                | Yes                         | Yes                        |                                               |
...
| ranger       | Terminal file manager                      | `pkg install ranger`              | Yes                         | Yes                        |                                               |
| neofetch     | System info screenshot tool                | `pkg install neofetch`            | Yes                         | Yes                        |                                               |
| fastfetch    | System info (neofetch alt)                 | `pkg install fastfetch`           | Yes                         | Yes                        |                                               |
| zoxide       | Directory jumper                           | `pkg install zoxide`              | Yes                         | Yes                        |                                               |
| starship     | Prompt theming                             | `pkg install starship`            | Yes                         | Yes                        |                                               |
| glow         | Markdown TUI viewer                        | `pkg install glow`                | Yes                         | Yes                        |                                               |
| exa/eza      | Modern ls replacement                      | `pkg install eza`                 | Yes                         | Yes                        |                                               |
