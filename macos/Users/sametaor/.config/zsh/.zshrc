...
# Lazyloading logic "borrowed" from the awesome qoomon, go check his repo out! https://github.com/qoomon/zsh-lazyload/blob/master/zsh-lazyload.zsh
function lazyload {  
  local seperator='--'
...
for fnfile in "$ZDOTDIR/functions"/**/*(N-.); do    # (N-.) = regular files, no errors
  case ${fnfile:t} in
    ...
    *.zsh)                 # classic 1-file-1-function style
      cmd=${fnfile:t:r}
      load_cmd="source ${(qqqq)fnfile}"
      lazyload $cmd -- $load_cmd
      ;;
  esac
done
