...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec2 center = uv_pattern - 0; // Use uv_pattern for centering the visual effects

    // Deus Ex Color Palette
    vec3 color_bg_dark = vec3(0.05, 0.07, 0.1);       // Muted dark blue/almost black
    vec3 color_blue_accent = vec3(1.0, 1.0, 1.0);  // Slightly brighter muted blue
    vec3 color_gold_accent = vec3(1.0, 0.55, 1.0);     // Muted gold
    vec3 color_grey_metallic = vec3(0.3, 0.35, 0.4);  // Grey metallic
    vec3 color_yellow_glow = vec3(1.0, 0.55, 1.0);     // Hint of cybernetic yellow
    ...
    for (float i = 0.0; i < 3.0; i++) // Create 3 layers
    {
        ...
        float flow_factor = fract(p.x * 0.5 + p.y * 0.3 - iTime * 0.2) * 0.5 + 0.5;
...
//      final_pattern_color = mix(final_pattern_color, layer_color * intensity * flow_factor, lines * (1.0 - smoothstep(0.4, 0.8, length(uv_pattern - 0.5))));
        final_pattern_color = mix(final_pattern_color, layer_color * intensity * flow_factor, lines * (1.0 - smoothstep(1, 1, length(uv_pattern - 2))));
    ...
    final_pattern_color = mix(final_pattern_color, color_grey_metallic, metallic_sheen * 0.0); // Subtle grey metallic overlay
