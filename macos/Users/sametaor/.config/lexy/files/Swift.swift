...
let emptyArray2 = Array() // same as above
// [T] is shorthand for Array
let emptyArray3: [String] = [] // Declaring the type explicitly allows you to set it to an empty array
let emptyArray4: Array = [] // same as above

// [Key: Value] is shorthand for Dictionary
let emptyDictionary = [String: Double]()
let emptyDictionary2 = Dictionary() // same as above
var emptyMutableDictionary: [String: Double] = [:]
var explicitEmptyMutableDictionary: Dictionary = [:] // same as above
...
/*
 ...
 Optional is an enum, with the cases .none (nil) and .some(T) (the value)
 */

var someOptionalString: String? = "optional" // Can be nil
// T? is shorthand for Optional — ? is a postfix operator (syntax candy)
let someOptionalString2: Optional = nil
...
// Pre-Swift 5, T! was shorthand for ImplicitlyUnwrappedOptional
// Swift 5 and later, using ImplicitlyUnwrappedOptional throws a compile-time error.
//var unwrappedString2: ImplicitlyUnwrappedOptional = "Value is expected." //error
...
func testGuard() {
    ...
    guard let aNumber = Optional(7) else {
